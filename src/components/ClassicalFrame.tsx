    ...
    <div className={`absolute ${rotationClass} z-20 pointer-events-none w-10 h-10 md:w-14 md:h-14 opacity-90 transition-opacity duration-300 group-hover:opacity-100 drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]`}>
      ...
      >
        ...
        <path
          ...
          fill="none"
        />
        <circle cx="15" cy="15" r="2.5" fill={`url(#goldGrad-${position})`} />
        <circle cx="28" cy="8" r="1.5" fill={`url(#goldGrad-${position})`} />
        <circle cx="8" cy="28" r="1.5" fill={`url(#goldGrad-${position})`} />
        {/* Corner Tip Pearl */}
        <circle cx="5" cy="5" r="2" fill="#fff9e6" />
      </svg>
    </div>
  );
};
...
const CenterMedallion: React.FC<{ isBottom?: boolean; color?: string }> = ({
  isBottom = false,
  color = '#d4af37',
}) => {
  return (
    ...
    >
      <svg viewBox="0 0 100 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
        <defs>
          <linearGradient id="medallionGrad" x1="0%" y1="0%" x2="100%" y2="0%">
        ...
        />
        {/* Center Cartouche */}
        <ellipse cx="50" cy="12" rx="7" ry="5" fill="#14120f" stroke="url(#medallionGrad)" strokeWidth="1.5" />
        <circle cx="50" cy="12" r="2.5" fill="#ffe699" />
        {/* Dots */}
        <circle cx="32" cy="12" r="1.5" fill="#ffe699" />
        <circle cx="68" cy="12" r="1.5" fill="#ffe699" />
      </svg>
    </div>
  );
};
...
export const ClassicalFrame: React.FC<ClassicalFrameProps> = ({
  ...
  return (
    ...
    >
      {/* Classical Outer Molded Gilded Frame */}
      <div
        className={`relative p-3 sm:p-4 md:p-5 rounded-sm transition-all duration-700 ease-out shadow-[0_20px_50px_rgba(0,0,0,0.9)] ${frameStylesConfig.glow}`}
        style={{
          background: `radial-gradient(ellipse at center, #1c1a16 0%, #0c0b09 85%, #050505 100%)`,
          border: '1px solid rgba(212, 175, 55, 0.4)',
        }}
      >
        {/* Layer 1: Ornamental Gilded Bevel (Carved Wood / Antique Foil Rim) */}
        <div className="absolute inset-0 rounded-sm p-[3px] pointer-events-none">
          <div
            ...
            style={{
              borderImage: `linear-gradient(135deg, #f7e6b5 0%, #ab822e 25%, #664913 50%, #d4af37 75%, #f7e6b5 100%) 1`,
            }}
          />
        </div>

        {/* Layer 2: Classical Corner Filigree Flourishes */}
        <CornerFlourish position="top-left" color={frameStylesConfig.filigreeColor} />
        <CornerFlourish position="top-right" color={frameStylesConfig.filigreeColor} />
        <CornerFlourish position="bottom-left" color={frameStylesConfig.filigreeColor} />
        <CornerFlourish position="bottom-right" color={frameStylesConfig.filigreeColor} />

        {/* Layer 3: Center Crest Medallions */}
        <CenterMedallion isBottom={false} color={frameStylesConfig.filigreeColor} />
        <CenterMedallion isBottom={true} color={frameStylesConfig.filigreeColor} />

        {/* Layer 4: Museum Matting (Passe-partout) with Deep Shadow */}
        <div className="relative p-2 md:p-3 rounded-[2px] bg-[#0c0c0e] shadow-[inset_0_4px_16px_rgba(0,0,0,0.95)] border border-[#2c261b]/60">
          
          {/* Layer 5: Inner Gold Fillet Liner (Thin Gilded Rim before artwork) */}
          <div className="relative border border-[#d4af37]/35 p-[2px] rounded-[1px] shadow-[inset_0_0_8px_rgba(0,0,0,0.8)] overflow-hidden">
            ...
            <div className="relative overflow-hidden w-full h-full rounded-[1px] bg-[#050505]">
              ...
              <div
                className="absolute inset-0 pointer-events-none opacity-0 group-hover:opacity-20 transition-opacity duration-700 bg-gradient-to-tr from-transparent via-white to-transparent"
                style={{
        ...
        {/* Optional Classical Museum Engraved Brass Plaque */}
        {showPlaque && (caption || subcaption) && (
          <div className="mt-3 text-center px-2 py-1.5 rounded-sm bg-gradient-to-b from-[#221e17] to-[#12100d] border border-[#a88434]/40 shadow-[0_4px_12px_rgba(0,0,0,0.7)] relative overflow-hidden">
            {/* Fine plaque screws */}
            <div className="absolute left-2 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-[#8c6d32] border border-[#f0d694]" />
            <div className="absolute right-2 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-[#8c6d32] border border-[#f0d694]" />

            {caption && (
              <h4 className="font-cinzel text-xs md:text-sm font-semibold tracking-widest text-[#f5dfa8] uppercase drop-shadow-[0_1px_2px_rgba(0,0,0,0.9)]">
                {caption}
              </h4>
            )}
            {subcaption && (
              <p className="font-cormorant italic text-[11px] md:text-xs text-[#d1baa0] mt-0.5 tracking-wider">
                {subcaption}
              </p>
            )}
          </div>
